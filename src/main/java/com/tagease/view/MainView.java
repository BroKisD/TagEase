...
public class MainView {
    ...
    private void showTagManagementWindow() {
        ...
        // Get non-system tags for the selector (moved up for use in both add and remove sections)
        List<String> nonSystemTags = controller.getAllTags().stream()
                .filter(name -> !Tag.isSystemTag(name))
                .collect(Collectors.toList());
                
        // Create the tag selector here so it can be referenced in both add and remove tag handlers
        ComboBox<String> tagSelector = new ComboBox<>();
        tagSelector.setPromptText("Select tag to remove");
        tagSelector.setItems(FXCollections.observableArrayList(nonSystemTags));
        ...
        addTagButton.setOnAction(e -> {
            ...
            if (!newTag.isEmpty()) {
                try {
                    ...
                    // Update the non-system tags list for the remove tag selector
                    List<String> updatedNonSystemTags = controller.getAllTags().stream()
                            .filter(name -> !Tag.isSystemTag(name))
                            .collect(Collectors.toList());
                    
                    // Update the tag selector with a new observable list
                    tagSelector.getSelectionModel().clearSelection();
                    tagSelector.setItems(FXCollections.observableArrayList(updatedNonSystemTags));
                    
                    // Refresh the UI after adding a tag
                    refreshTable();
                    
                    // Only update tag filter box if it's initialized
                    if (tagFilterBox != null) {
                        updateTagFilterBox();
                    }
                } catch (Exception ex) {
                    showErrorDialog("Error", "Failed to add tag", ex.getMessage());
                }
            }
        });
        ...
        removeTagButton.setOnAction(e -> {
            ...
            if (selectedTag != null) {
                // Clear selection before modifying the list to avoid index errors
                tagSelector.getSelectionModel().clearSelection();
                
                // Remove the tag
                controller.removeTag(selectedTag);
                
                // Update the tag selector with the current list of tags
                List<String> updatedNonSystemTags = controller.getAllTags().stream()
                        .filter(name -> !Tag.isSystemTag(name))
                        .collect(Collectors.toList());
                
                // Update items with a new observable list to avoid selection issues
                ObservableList<String> newItems = FXCollections.observableArrayList(updatedNonSystemTags);
                tagSelector.setItems(newItems);
                
                // Refresh the UI after removing a tag
                refreshTable();
    ...
    private void updateTagFilterBox() {
        try {
            // Check if tagFilterBox is initialized
            if (tagFilterBox == null) {
                return; // Exit the method if tagFilterBox is null
            }
            ...
            ObservableList<Object> newItems = FXCollections.observableArrayList();
            Set<String> allTags = tagColorMap.keySet();
            
            // Create all the items first before adding them to the list
            for (String tagName : allTags) {
                ...
                final String finalTagName = tagName; // Create a final copy for the lambda
                checkBox.setOnAction(e -> {
                    if (checkBox.isSelected()) {
                        selectedTags.add(finalTagName);
                    } else {
                        selectedTags.remove(finalTagName);
                    }
                    updateSelectedTagsDisplay();
                    refreshTable();
                });
                
                // Add to our new items list
                newItems.add(itemToAdd);
            }
            
            // Update the ListView with the new items all at once
            Platform.runLater(() -> {
                try {
                    // Disable selection temporarily
                    MultipleSelectionModel<?> selectionModel = tagFilterBox.getSelectionModel();
                    if (selectionModel != null) {
                        selectionModel.clearSelection();
                    }
                    
                    // Replace all items at once
                    tagFilterBox.setItems(newItems);
                    
                    // Set a custom cell factory that prevents selection on empty lists
                    tagFilterBox.setCellFactory(listView -> {
                        ...
                        // Prevent selection if the list is empty or index is invalid
                        cell.setOnMousePressed(event -> {
                            if (tagFilterBox.getItems().isEmpty() || cell.getIndex() < 0 || cell.getIndex() >= tagFilterBox.getItems().size()) {
                    ...
                    // Disable selection model to prevent IndexOutOfBoundsException
                    tagFilterBox.getSelectionModel().setSelectionMode(SelectionMode.SINGLE);
                    tagFilterBox.getSelectionModel().selectedItemProperty().addListener((obs, oldVal, newVal) -> {
                        // If we try to select an item in an empty list, clear selection
                        if (newVal != null && tagFilterBox.getItems().isEmpty()) {
                            Platform.runLater(() -> tagFilterBox.getSelectionModel().clearSelection());
                        }
                    });
                } catch (Exception ex) {
                    System.err.println("Error updating tag filter box: " + ex.getMessage());
                    ex.printStackTrace();
                }
            });
    ...
    private void editFileTags(TaggedFile file) {
        ...
        // Create a set to track the current tags (will be modified when removing tags)
        final Set<String> fileTags = new HashSet<>(file.getTags());
        
        // Current tags section
        Label currentTagsLabel = new Label("Current Tags:");
        currentTagsLabel.getStyleClass().add("section-label");

        // Create the FlowPane for current tags
        final FlowPane currentTagsPane = new FlowPane();
        currentTagsPane.setHgap(8);
        currentTagsPane.setVgap(8);
        currentTagsPane.setPadding(new Insets(5));
        
        // Method to update the current tags display
        class TagsUpdater {
            void updateCurrentTagsDisplay() {
                ...
                for (String tagName : fileTags) {
                    ...
                    // Apply the tag's specific color (with 20% opacity for background)
                    tagBox.setStyle("-fx-background-color: " + colorHex + "33;"); // 33 = 20% opacity
                    ...
                    Button removeButton = new Button("×");
                    removeButton.getStyleClass().add("remove-tag-button");
                    removeButton.setMinSize(16, 16);
                    removeButton.setPrefSize(16, 16);
                    removeButton.setMaxSize(16, 16);
                    removeButton.setPadding(new Insets(0, 0, 0, 0));
                    removeButton.setStyle("-fx-font-size: 1px;");
                    
                    // Use a final variable for the tag name to use in lambda
                    final String tagToRemove = tagName;
                    removeButton.setOnAction(e -> {
                        fileTags.remove(tagToRemove);
                        updateCurrentTagsDisplay();
                    });
                    
                    tagBox.getChildren().addAll(tagLabel, removeButton);
                    currentTagsPane.getChildren().add(tagBox);
                }
            }
        }
        
        // Create the updater and initialize the display
        final TagsUpdater tagsUpdater = new TagsUpdater();
        tagsUpdater.updateCurrentTagsDisplay();

        // Add new tags section
        Label addTagsLabel = new Label("Add New Tags:");
        addTagsLabel.getStyleClass().add("section-label");
        
        TextField newTagField = new TextField();
        newTagField.setPromptText("Type a new tag and press Enter");
        newTagField.setPrefHeight(35);
        ...
        for (String tagName : allTagNames) {
            ...
            // Use a final variable for the tag name to use in lambda
            final String tagToAdd = tagName;
            tagButton.setOnAction(e -> {
                // Add the tag if it's not already in the file tags
                if (!fileTags.contains(tagToAdd)) {
                    fileTags.add(tagToAdd);
                    tagsUpdater.updateCurrentTagsDisplay();
                }
            });
        ...
        dialog.setResultConverter(buttonType -> {
            if (buttonType == saveButton) {
                // Update the file's tags with the modified set
